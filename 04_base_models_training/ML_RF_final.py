"""
Final Random Forest bias correction with station-grouped five-fold OOF.
...
# Optimal RF hyperparameters from the completed search
PARAMS = {
    "criterion": "squared_error", "n_estimators": 800, "max_depth": 40,
...
for fold in range(1, N_SPLITS + 1):
    ...
    pred["Fold_ID"], pred["Model"] = fold, "RandomForest"
    ...
    histories.append({"Fold_ID": fold, "Training_Seconds": elapsed,
...
# -------------------- Save outputs --------------------
fold_metrics.to_csv(OUT / "RF_fold_metrics.csv", index=False)
oof.to_csv(OUT / "RF_OOF_predictions.csv", index=False)
overall.to_csv(OUT / "RF_OOF_overall_metrics.csv", index=False)
station_metrics.to_csv(OUT / "RF_OOF_station_metrics.csv", index=False)
history.to_csv(OUT / "RF_training_history.csv", index=False)
params_out.to_csv(OUT / "RF_best_parameters.csv", index=False)
test_pred.to_csv(OUT / "RF_final_test_predictions.csv", index=False)
test_metrics.to_csv(OUT / "RF_final_test_metrics.csv", index=False)
