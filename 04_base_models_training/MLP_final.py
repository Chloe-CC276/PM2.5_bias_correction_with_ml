"""
Final MLP bias correction with station-grouped five-fold OOF.

Uses the shared 63/16 station split and the width/depth selected during
hyperparameter search. The independent test set is evaluated once.
"""
...
RESULTS_ROOT = DATA.parents / "results"
...
def build_model(n_features, params):
    """Use the same gradually decreasing architecture as MLP tuning."""
    model = Sequential([Input(shape=(n_features,), name="features")])
    for layer_id in range(params["n_layers"]):
        layer_units = max(int(params["units"] / (2**layer_id)), 16)
        model.add(
            Dense(
                layer_units,
...
def metrics(obs, raw, corrected):
    """Use exactly the same before/after definitions as the XGBoost code."""
...
def prediction_table(data, predicted_bias, config, fold_id=None):
    """Create an XGBoost-aligned prediction table using stable row_id."""
    ...
    out["Model"], out["Config"] = "MLP", config
...
def load_aligned_data():
    """Load geo features and reuse the XGBoost 63/16 split and 5 folds."""
    df = pd.read_csv(DATA)
    ...
    if not required_geo.issubset(features) or len(features) != 36:
        raise ValueError(f"Expected 36 features including grid_lat/grid_lon: {features}")

    if FOLD_FILE.exists():
        folds = pd.read_csv(FOLD_FILE, dtype={"row_id": str})
        if set(dev["row_id"]) != set(folds["row_id"]):
            raise ValueError(
                "The geo dataset rows do not exactly match the XGBoost development folds."
            )
        dev = dev.merge(
...
def run_configuration(dev, test, features, params):
    ...
    for fold in range(1, N_SPLITS + 1):
        ...
        x_train = scaler.fit_transform(train_fold[features]).astype(np.float32)
        x_val = scaler.transform(val_fold[features]).astype(np.float32)
    ...
    x_dev = final_scaler.fit_transform(dev[features]).astype(np.float32)
    x_test = final_scaler.transform(test[features]).astype(np.float32)
    ...
    fold_metrics.to_csv(out / "MLP_fold_metrics.csv", index=False)
    oof.to_csv(out / "MLP_OOF_predictions.csv", index=False)
    overall.to_csv(out / "MLP_OOF_overall_metrics.csv", index=False)
    station_metrics.to_csv(out / "MLP_OOF_station_metrics.csv", index=False)
    pd.concat(histories, ignore_index=True).to_csv(
        out / "MLP_training_history.csv", index=False
    )
    params_out.to_csv(out / "MLP_best_parameters.csv", index=False)
    test_pred.to_csv(out / "MLP_final_test_predictions.csv", index=False)
    test_metrics.to_csv(out / "MLP_final_test_metrics.csv", index=False)

    print("\n>>> OOF overall metrics (use for model selection):")
    print(overall.to_string(index=False))
    print("\n>>> Test metrics (preview only; do not use for selection):")
