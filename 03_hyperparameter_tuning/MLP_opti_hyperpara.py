"""
Two-stage hyperparameter search for an MLP bias-correction model.

Stage 1 is a broad random search; stage 2 is a local grid over hidden width
and depth. Search uses an 80,000-row subsample from development stations.
"""
...
def build_model(n_features, units, n_layers, activation, dropout,
                learning_rate, l2_value):
    """Build an MLP; hidden widths gradually decrease in deeper layers."""
    model = Sequential([Input(shape=(n_features,))])
    for layer in range(n_layers):
        layer_units = max(int(units / (2 ** layer)), 16)
        model.add(Dense(
...
def train_and_score(params, X_train, y_train, X_val, val_df):
    tf.keras.backend.clear_session()
    model = build_model(
        X_train.shape[1], units=params["units"], n_layers=params["n_layers"],
    ...
    history = model.fit(
        X_train, y_train,
        validation_data=(X_val, val_df[TARGET].to_numpy()),
    ...
    obs, musica = val_df[OBS].to_numpy(), val_df[MUSICA].to_numpy()
...
def plot_results(results):
    ...
    cmaps = ["YlOrRd", "YlOrRd", "YlOrRd", "Blue"]
    fig, axes = plt.subplots(1, 4, figsize=(25, 6), dpi=200)
    for ax, name, cmap_name in zip(axes, metric_names, cmaps):
        table = results.pivot(index="n_layers", columns="units", values=name)
        sns.heatmap(table, annot=True, fmt=".4f", cmap=cmap_name,
                    linewidths=0, ax=ax, cbar_kws={"label": name})
        ax.set_title(f"MLP Hyperparameter Tuning ({name})", fontweight="bold")
        ax.set_xlabel("Units in first hidden layer")
        ax.set_ylabel("Number of hidden layers")
    plt.tight_layout()
    plt.savefig(FIG / "MLP_units_n_layers_heatmap.png", dpi=300)
    plt.close()

    units_values = np.sort(results["units"].unique())
    layer_values = np.sort(results["n_layers"].unique())
    x, y = np.meshgrid(units_values, layer_values)
    fig = plt.figure(figsize=(25, 7), dpi=200)
    for i, name in enumerate(metric_names, 1):
        z = (results.pivot(index="n_layers", columns="units", values=name)
             .reindex(index=layer_values, columns=units_values).to_numpy())
        ax = fig.add_subplot(1, 4, i, projection="3d")
        surf = ax.plot_surface(x, y, z, cmap=cm.viridis, edgecolor="black",
                               linewidth=0.35, antialiased=True)
        ax.set_title(f"MLP Response ({name})", fontweight="bold")
        ax.set_xlabel("Units")
    ...
    plt.tight_layout()
    plt.savefig(FIG / "MLP_units_n_layers_3D.png", dpi=300)
    plt.close()


def compare_geo_oof(data, geo_features, params):
    """Print five-fold spatial OOF metrics with and without coordinates."""
    no_geo_features = [c for c in geo_features if c not in {"grid_lat", "grid_lon"}]
    oof_geo = np.empty(len(data), dtype=np.float32)
    oof_no_geo = np.empty(len(data), dtype=np.float32)
    splitter = GroupKFold(n_splits=5)

    print("\n" + "=" * 72)
    print("Five-fold spatial OOF ablation: with coordinates vs without coordinates")
    print("Same folds, best hyperparameters and training epochs are used.")

    for fold, (train_idx, val_idx) in enumerate(
            splitter.split(data, groups=data[STATION]), 1):
        train_fold, val_fold = data.iloc[train_idx], data.iloc[val_idx]

        for use_geo, feature_set, output in [
            (True, geo_features, oof_geo),
            (False, no_geo_features, oof_no_geo),
        ]:
            scaler_fold = StandardScaler()
            X_fold_train = scaler_fold.fit_transform(
                train_fold[feature_set]
            ).astype(np.float32)
            X_fold_val = scaler_fold.transform(
                val_fold[feature_set]
            ).astype(np.float32)

            tf.keras.backend.clear_session()
            tf.keras.utils.set_random_seed(SEED + fold)
            fold_model = build_model(
                len(feature_set), units=params["units"],
                n_layers=params["n_layers"], activation=params["activation"],
                dropout=params["dropout"], learning_rate=params["learning_rate"],
                l2_value=params["l2"]
            )
            fold_model.fit(
                X_fold_train, train_fold[TARGET].to_numpy(dtype=np.float32),
                epochs=params["best_epoch"], batch_size=params["batch_size"],
                verbose=0, shuffle=True
            )
            output[val_idx] = fold_model.predict(
                X_fold_val, batch_size=params["batch_size"], verbose=0
            ).ravel()

        obs = val_fold[OBS].to_numpy()
        musica = val_fold[MUSICA].to_numpy()
        fold_geo = metrics(obs, musica, oof_geo[val_idx])
        fold_no_geo = metrics(obs, musica, oof_no_geo[val_idx])
        print(f"\nFold {fold} | validation stations={val_fold[STATION].nunique()}")
        print("  Without geo | " + " | ".join(
            f"{name}={fold_no_geo[name]:.4f}" for name in ["R2", "MAE", "RMSE", "BIAS"]
        ))
        print("  With geo    | " + " | ".join(
            f"{name}={fold_geo[name]:.4f}" for name in ["R2", "MAE", "RMSE", "BIAS"]
        ))

    obs_all, musica_all = data[OBS].to_numpy(), data[MUSICA].to_numpy()
    overall_no_geo = metrics(obs_all, musica_all, oof_no_geo)
    overall_geo = metrics(obs_all, musica_all, oof_geo)
    print("\n" + "-" * 72)
    print("Overall OOF metrics across all 63 development stations")
    print("Without geo | " + " | ".join(
        f"{name}={overall_no_geo[name]:.4f}" for name in ["R2", "MAE", "RMSE", "BIAS"]
    ))
    print("With geo    | " + " | ".join(
        f"{name}={overall_geo[name]:.4f}" for name in ["R2", "MAE", "RMSE", "BIAS"]
    ))
    print("Geo - NoGeo | " + " | ".join(
        f"Delta_{name}={overall_geo[name] - overall_no_geo[name]:+.4f}"
        for name in ["R2", "MAE", "RMSE", "BIAS"]
    ))
    print("Interpretation: higher Delta_R2 and lower Delta_MAE/RMSE are better.")
    print("For BIAS, compare which absolute value is closer to zero.")
    print("=" * 72)
...
required_geo = {"grid_lat", "grid_lon"}
if not required_geo.issubset(features):
    raise ValueError("The geographic features grid_lat and grid_lon are required.")
if len(features) != 36:
    raise ValueError(f"Expected 36 features, found {len(features)}: {features}")
...
X_train = scaler.fit_transform(sample[features]).astype(np.float32)
X_val = scaler.transform(val_current[features]).astype(np.float32)
...
space = {
    "units": [32, 64, 96, 128, 192, 256, 384, 512],
    "n_layers": [1, 2, 3, 4],
...
while len(random_rows) < N_RANDOM_TRIALS:
    ...
    pd.DataFrame(random_rows).to_csv(OUT / "MLP_random_search_results.csv", index=False)

random_results = pd.DataFrame(random_rows).sort_values("MAE").reset_index(drop=True)
best = random_results.iloc[0]


# -------------------- Stage 2: fine units x n_layers grid --------------------
fixed = {
...
units_values = [96, 128, 192, 256, 384, 512, 768, 1024, 1536]
layer_values = [2, 3, 4, 5, 6, 7]
fine_rows = []
for units in units_values:
    for n_layers in layer_values:
        params = {**fixed, "units": units, "n_layers": n_layers}
        print(f"\n[Fine grid] units={units} | n_layers={n_layers}")
        scores = train_and_score(params, X_train, y_train, X_val, val_current)
        fine_rows.append({**params, **scores})
        print(f"R2={scores['R2']:.4f} | MAE={scores['MAE']:.4f} | "
              f"RMSE={scores['RMSE']:.4f} | BIAS={scores['BIAS']:.4f}")
        pd.DataFrame(fine_rows).to_csv(OUT / "MLP_fine_grid_results.csv", index=False)

fine_results = pd.DataFrame(fine_rows).sort_values("MAE").reset_index(drop=True)
fine_results.to_csv(
    OUT / "MLP_best_validation_before_after_metrics.csv", index=False
)
best_fine = fine_results.iloc[0]
best_params = {
    "units": int(best_fine["units"]),
    "n_layers": int(best_fine["n_layers"]),
    "activation": str(best_fine["activation"]),
...
with open(OUT / "MLP_best_parameters.json", "w", encoding="utf-8") as file:
    json.dump(best_params, file, indent=2)


# -------------------- Retrain best MLP and save validation predictions --------------------
tf.keras.backend.clear_session()
tf.keras.utils.set_random_seed(SEED)
final_model = build_model(
    len(features), units=best_params["units"], n_layers=best_params["n_layers"],
...
prediction_output.to_csv(OUT / "MLP_best_validation_predictions.csv", index=False)
...
pd.DataFrame([{**best_params, **best_summary}]).to_csv(
    OUT / "MLP_best_validation_summary.csv", index=False
)

plot_results(pd.DataFrame(fine_rows))
compare_geo_oof(dev.reset_index(drop=True), features, best_params)
print("\nBest MLP parameters:")
